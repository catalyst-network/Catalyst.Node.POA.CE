    ...
    public sealed class ConfigurableRepoIntegrationTests : FileSystemBasedTest
    {
        ...
        private async Task<string> PointXmlConfigToLocalTestFolder(string mempoolConfigFile)
        {
            var originalContent = await
                File.ReadAllTextAsync(Path.Combine(Constants.ConfigSubFolder, mempoolConfigFile));
            var newContent =
        ...
        {
            var fi = new FileInfo(Path.Combine(Constants.ConfigSubFolder,
                "InMemoryTestingRepository.json"));
